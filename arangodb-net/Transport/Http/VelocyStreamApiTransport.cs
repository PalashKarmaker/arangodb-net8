...
public class VelocyStreamApiTransport(
    HttpClient client,
    bool suppressClientDisposal = false) : IApiClientTransport
{
    private static readonly string contentType = "application/x-velocypack";
    ...
    /// <summary>
    /// Get an instance of <see cref="VelocyStreamApiTransport"/> that uses no authentication.
    /// </summary>
    /// <param name="hostUri"></param>
    /// <param name="dbName"></param>
    /// <param name="encodedCA">Base64 encoded CA certificate</param>
    /// <returns></returns>
    public static VelocyStreamApiTransport UsingNoAuth(
        Uri hostUri,
        ...
        string encodedCA = null)
    {
        ...
        var transport = new VelocyStreamApiTransport(client);
        return transport;
    }

    /// <summary>
    /// Get an instance of <see cref="VelocyStreamApiTransport"/> that uses basic
    ...
    /// <param name="encodedCA">Base64 encoded CA certificate</param>
    /// <returns></returns>
    public static VelocyStreamApiTransport UsingBasicAuth(
        Uri hostUri,
        string username,
        string password,
        string encodedCA = null)
    {
        return UsingBasicAuthWithDbName(hostUri, "_system", username, password, encodedCA);
    }

    /// <summary>
    /// Get an instance of <see cref="VelocyStreamApiTransport"/> that uses basic auth.
    /// </summary>
    /// <param name="hostUri"></param>
    /// <param name="dbName"></param>
    /// <param name="username"></param>
    ...
    /// <param name="encodedCA">Base64 encoded CA certificate</param>
    /// <returns></returns>
    public static VelocyStreamApiTransport UsingBasicAuthWithDbName(
        Uri hostUri,
        string dbName,
        string username,
        string password,
    ...
    /// <summary>
    /// Get an instance of <see cref="VelocyStreamApiTransport"/> that uses
    ...
    /// <param name="encodedCA">Base64 encoded CA certificate</param>
    /// <returns></returns>
    public static VelocyStreamApiTransport UsingJwtAuth(
        Uri hostUri,
        string jwtToken,
        HttpContentType contentType = HttpContentType.Json,
        string encodedCA = null)
    {
        return UsingJwtAuth(hostUri, "_system", jwtToken, encodedCA);
    }
    ...
    /// <summary>
    /// Make this <see cref="VelocyStreamApiTransport"/> instance use JSON content type
    ...
    /// <summary>
    /// Make this <see cref="VelocyStreamApiTransport"/> instance use VPack content type
    /// for Content-Type and Accept HTTP headers.
    /// </summary>
        ...
        CancellationToken token = default)
    {
        ...
        var response = await client.PostAsync(requestUri, httpContent, token).ConfigureAwait(false);
        return new HttpApiClientResponse(response);
    }
        ...
        CancellationToken token = default)
    {
        ...
        httpContent.Headers.ContentType = new MediaTypeHeaderValue(contentType);
        ApplyHeaders(webHeaderCollection, httpContent.Headers, DriverFlags);
        var response = await client.PutAsync(requestUri, httpContent, token).ConfigureAwait(false);
        return new HttpApiClientResponse(response);
    }
